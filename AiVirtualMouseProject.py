...
while cap.isOpened():
    ...
    if len(lmList) != 0:
        ...
        # 通过食指下按的速率来实现单击左键操作
        currIF_y = p8_y
        if prevIF_y >= 0 and fps > 0.0:
            if (currIF_y - prevIF_y) * fps / hCam > 1.1:
                # print((currIF_y - prevIF_y) * fps / hCam)
                pyautogui.click()
        prevIF_y = currIF_y

        # 通过中指下按的速率来实现单击右键操作
        currMF_y = p12_y
        if prevMF_y >= 0 and fps > 0.0:
            if (currMF_y - prevMF_y) * fps / hCam > 1.8:
                # print((currMF_y - prevMF_y) * fps / hCam)
